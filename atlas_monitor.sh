...
do
	...
	if [ -d /data/data/com.pokemod.atlas ] && [ -s /data/local/tmp/atlas_config.json ]
		then
			...
			[[ ! -z $discord_webhook ]] && [[ $recreate_atlas_config != "false" ]] && curl -S -k -L --fail --show-error -F "payload_json={\"content\": \"__**$origin**__: `atlas_config.json missing. Recreating.`\"}" $discord_webhook &>/dev/null
	...
	then
		...
		[[ ! -z $discord_webhook ]] && curl -S -k -L --fail --show-error -F "payload_json={\"content\": \"__**$origin**__: `Device not licensed. Check Atlas Dashboard`\"}" $discord_webhook &>/dev/null
	...
	then
		...
		send_webhook "Atlas Dead" "Reboot"
   	        [[ ! -z $discord_webhook ]] && [[ $atlas_died != "false" ]] && curl -S -k -L --fail --show-error -F "payload_json={\"content\": \"__**$origin**__: `Atlas was rebooted.`\"}" $discord_webhook &>/dev/null
                reboot
		...
		send_webhook "Pogo Dead" "Reboot"
   	        [[ ! -z $discord_webhook ]] && [[ $pogo_died != "false" ]] && curl -S -k -L --fail --show-error -F "payload_json={\"content\": \"__**$origin**__: `Pogo was rebooted.`\"}" $discord_webhook &>/dev/null
                reboot
	...
	then
		...
		[[ ! -z $discord_webhook ]] && curl -S -k -L --fail --show-error -F "payload_json={\"content\": \"__**$origin**__: `Unknown Error happened on device.`\"}" $discord_webhook &>/dev/null
	fi
	sleep $monitor_interval
done
